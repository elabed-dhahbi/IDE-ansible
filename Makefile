...
help:
	...
	@echo "  $(GREEN)sync-packages$(NC)              - Scan offline directory and auto-update versions"
	@echo "  $(GREEN)prepare$(NC)                    - Sync packages, then validate offline packages and system readiness"
	@echo "  $(GREEN)install$(NC)                     - Full installation (all components)"
	@echo "  $(GREEN)install TAG=xxx$(NC)            - Install specific component(s) by tag"
	@echo ""
	@echo "$(YELLOW)=== Tag-Based Installation (make install TAG=xxx) ===$(NC)"
	@echo "  $(GREEN)make install TAG=preflight$(NC)         - Preflight checks only"
	@echo "  $(GREEN)make install TAG=postgres$(NC)           - PostgreSQL only"
	@echo "  $(GREEN)make install TAG=opensearch$(NC)         - OpenSearch only"
	@echo "  $(GREEN)make install TAG=logstash-oss$(NC)      - Logstash OSS only"
	@echo "  $(GREEN)make install TAG=opensearch_dashboards$(NC) - OpenSearch Dashboards only"
	@echo "  $(GREEN)make install TAG=elasticsearch$(NC)      - Elasticsearch only"
	@echo "  $(GREEN)make install TAG=logstash$(NC)           - Logstash only"
	@echo "  $(GREEN)make install TAG=kibana$(NC)             - Kibana only"
	@echo "  $(GREEN)make install TAG=dmc1$(NC)               - DMC Node 1 only"
	@echo "  $(GREEN)make install TAG=dmc2$(NC)               - DMC Node 2 only"
	@echo "  $(GREEN)make install TAG=dmc$(NC)                - DMC (both nodes) only"
	@echo "  $(GREEN)make install TAG=drs$(NC)                - DRS only"
	@echo "  $(GREEN)make install TAG=elkstatistic$(NC)      - ELK Statistics only"
	@echo "  $(GREEN)make install TAG=sls$(NC)                - SLS (core service) only"
	@echo "  $(GREEN)make install TAG=mmg$(NC)                - MMG only"
	@echo "  $(GREEN)make install TAG=mmsoap$(NC)             - MMSOAP only"
	@echo "  $(GREEN)make install TAG=smppc$(NC)              - SMPPC only"
	@echo "  $(GREEN)make install TAG=zabbix$(NC)             - Zabbix server only"
	@echo "  $(GREEN)make install TAG=zabbix-agents$(NC)      - Zabbix agents only"
	@echo "  $(GREEN)make install TAG=haproxy$(NC)           - HAProxy only"
	@echo ""
	@echo "$(YELLOW)=== Module Installation Targets ===$(NC)"
	...
	@echo ""
	@echo "$(YELLOW)=== Stack-Only Targets ===$(NC)"
	...
	@echo ""
	@echo "$(YELLOW)=== Individual Component Targets ===$(NC)"
	...
	@echo ""
	@echo "$(YELLOW)=== Resume Installation Targets ===$(NC)"
	...
	@echo "  $(GREEN)validate-config$(NC)            - Validate configuration files"
	...
	@echo ""
	@echo "$(YELLOW)Basic Workflow:$(NC)"
	@echo "  $(GREEN)make sync-packages$(NC)                     # 1. Update versions from offline directory"
	@echo "  $(GREEN)make prepare$(NC)                           # 2. Validate packages and system readiness"
	@echo "  $(GREEN)make install$(NC)                           # 3. Full installation (all components)"
	@echo ""
	@echo "$(YELLOW)Tag-Based Installation Examples:$(NC)"
	...
	@echo "  $(GREEN)make install TAG=mmg$(NC)                  # Install only MMG"
	@echo "  $(GREEN)make install TAG=mmsoap$(NC)                # Install only MMSOAP"
	@echo "  $(GREEN)make install TAG=smppc$(NC)                 # Install only SMPPC"
	@echo ""
	@echo "$(YELLOW)Environment-Specific Examples:$(NC)"
	...
	@echo "  $(GREEN)make prepare ENVIRONMENT=DEV$(NC)           # Prepare for DEV environment"
	@echo ""
	@echo "$(YELLOW)Module Installation Examples:$(NC)"
	@echo "  $(GREEN)make install-sls$(NC)                       # Install SLS standalone (for license upload)"
	@echo "  $(GREEN)make install-dmc-module-e SKIP_POSTGRES=1$(NC)  # Install DMC with ELK, skip PostgreSQL"
	@echo "  $(GREEN)make install-dmc-module-o SKIP_POSTGRES=1$(NC)  # Install DMC with OpenSearch, skip PostgreSQL"
	@echo ""
	@echo "$(YELLOW)Utility Examples:$(NC)"
	@echo "  $(GREEN)make check-inventory$(NC)                   # Test connectivity to all hosts"
	@echo "  $(GREEN)make list-packages$(NC)                     # List all files in offline directory"
	@echo "  $(GREEN)make status$(NC)                            # Show current environment status"
	@echo ""
